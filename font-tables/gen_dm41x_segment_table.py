#!/usr/bin/env python3
"""Rasterize Magellan's HP-41 segment geometry into the DM41X pixel table.
...
DM41X-style display's view modes: a "classic-line" view that reuses the
...
plan file's "DM41X-style Sharp Memory LCD backend" section for the full
...
fixed-column convention - see hp41_dm41x_font_table.h's header comment
for the real-world mm math this is derived from.

Usage (run from font-tables/, or anywhere - paths below are relative to
this script's directory):
    python3 gen_dm41x_segment_table.py > hp41_dm41x_font_table.c
...
# This table's own grid geometry - must match hp41_dm41x_font_table.h
...
NUM_ASCII_CODES = 128  # hp41_dm41x_char_segments[128]'s size, matching hp41_display_tables.h's
...
def check_geometry_matches_header() -> None:
    """Verify this script's constants agree with hp41_dm41x_font_table.h.
    ...
    """
    header = (SCRIPT_DIR / "hp41_dm41x_font_table.h").read_text()
    expected = {
        "HP41_DM41X_DISP_WIDTH_PX": DISP_WIDTH_PX,
        "HP41_DM41X_DISP_HEIGHT_PX": DISP_HEIGHT_PX,
        "HP41_DM41X_NUM_COLS": NUM_COLS,
        "HP41_DM41X_NUM_ROWS": NUM_ROWS,
        "HP41_DM41X_CELL_WIDTH_PX": CELL_WIDTH_PX,
        "HP41_DM41X_CELL_HEIGHT_PX": CELL_HEIGHT_PX,
        "HP41_DM41X_COL_PITCH_PX": COL_PITCH_PX,
        "HP41_DM41X_ROW_PITCH_PX": ROW_PITCH_PX,
        "HP41_DM41X_GRID_X0": GRID_X0,
        "HP41_DM41X_GRID_Y0": GRID_Y0,
    }
    for name, value in expected.items():
        ...
        check(match is not None and int(match.group(1)) == value,
              f"hp41_dm41x_font_table.h's {name} doesn't match this script's "
              f"value of {value} - update whichever one is stale")

    # Same fit-check hp41_dm41x_font_table.h documents as "verified at
...
def emit_c(d: DisplayTable) -> None:
    """Print hp41_dm41x_font_table.c's full contents to stdout.

    Args:
        d: Computed by compute_table().
    """
    print("// Auto-generated by gen_dm41x_segment_table.py from the Magellan")
    print(f"// project's data/segments.py and data/charset_41.py ({MAGELLAN_DIR}).")
    print("// Do not hand-edit - re-run the script instead.")
    print('#include "hp41_dm41x_font_table.h"')
    print()

    print(f"// Populated codes (all others are zero/blank): {d['populated']}", file=sys.stderr)
    print("// Bit i set -> segment SEGMENT_ORDER[i] (see hp41_dm41x_font_table.h) is lit.")
    ...
    print(f"const uint16_t hp41_dm41x_char_segments[{NUM_ASCII_CODES}] = {{")
    ...
    print("};")
    print(f"const uint16_t hp41_dm41x_segment_pixel_count[{NUM_SEGMENTS}] = {{")
