/**
 * @file hp41_arduino_bridge.h
 * @brief Dormant fallback display path: sends display state to an
 *        Arduino Uno over UART, which drives the LCD itself.
 *
 * See CLAUDE.md "Arduino display bridge" section: this was the interim
 * display path before three level shifter boards made the direct
 * Pico->LCD parallel link (st7920.c) possible. It sends the computed
 * framebuffer (or, more efficiently, the raw display registers) to an
 * Arduino Uno over UART, which drives the LCD directly via its own
 * already-hardware-validated parallel driver
 * (Arduino NHD14432/NHD14432_DisplayBridge.ino). Kept fully intact,
 * not deleted, in case the direct link ever needs to be bypassed again
...
/**
 * @brief Initialize the UART link to the Arduino bridge.
 */
void hp41_arduino_bridge_init(void);

/**
 * @brief Send a full pixel framebuffer to the Arduino for direct display.
 *
 * Frames it as [0xAA sync][576 payload bytes][1 XOR-checksum byte] -
 * the Arduino silently drops the frame if the checksum doesn't match,
 * rather than showing a corrupted image.
 *
 ...
 *
 * @param fb Framebuffer to send, exactly LCD_FB_SIZE bytes (see st7920.h).
 */
void hp41_arduino_bridge_send_frame(const uint8_t *fb);

/**
 * @brief Send the Nut CPU's raw display registers to the Arduino.
 *
...
/**
 * @brief TEMPORARY diagnostic: send a hardcoded, known-good display payload.
 *
 * Sends a hardcoded, captured-from-a-real-run 38-byte payload (the
 * "2.0000" state, captured via the raw-payload hex dump added to
 * hp41_arduino_bridge_send_display_state()) directly, with no Nut
 * CPU/ROM involvement at all - isolates whether this specific content
 * renders correctly on the Arduino when sent in isolation, free of any
 * burst/timing pressure from the ROM's normal 3-frames-per-key
