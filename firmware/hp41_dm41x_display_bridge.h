/**
 * @file hp41_dm41x_display_bridge.h
 * @brief Computes a framebuffer for the DM41X-style 400x240 Sharp
 *        Memory LCD display, in either of its two view modes.
 *
 * Phase 3b of the Magellan/DM41X plan
 ...
 * on real hardware in dm41x_bringup/ - see CLAUDE.md), cleared to
 * 0xFF (all-white), where a lit (visible, dark) segment is drawn by
 * *clearing* its bit, not setting it.
 */
#ifndef SOYNUT_HP41_DM41X_DISPLAY_BRIDGE_H
#define SOYNUT_HP41_DM41X_DISPLAY_BRIDGE_H
 ...
 *  dm41x/ build target) never needs to depend on dm41x_bringup/'s
 *  vendored subtree. */
#define HP41_DM41X_FB_SIZE \
    (((HP41_DM41X_DISP_WIDTH_PX + 7) / 8) * HP41_DM41X_DISP_HEIGHT_PX)

/**
 * @brief Which of the display's two view modes to render.
 *
 * Toggled by a new one-shot bridge command (see hp41_key_bridge.h's
 * "[DSP]" escape, Phase 3c) - mirrors the DM41X's own dedicated DSP key,
 ...
 *
 * @param fb   Output buffer, at least HP41_DM41X_FB_SIZE bytes; fully
...
#endif // SOYNUT_HP41_DM41X_DISPLAY_BRIDGE_H
