/**
 * @file hp41_dm41x_display_bridge.c
 * @brief Implements hp41_dm41x_display_compute_framebuffer() - see
 *        hp41_dm41x_display_bridge.h for the public contract.
 */
...
#define HP41_DM41X_NUM_CLASSIC_CELLS 12
...
extern unsigned char lcd_a[HP41_DM41X_NUM_CLASSIC_CELLS];
extern unsigned char lcd_b[HP41_DM41X_NUM_CLASSIC_CELLS];
extern unsigned char lcd_c[HP41_DM41X_NUM_CLASSIC_CELLS];
extern int lcd_ann;
 ...
 * dm41x_bringup/ (see CLAUDE.md's "Sharp Memory LCD bring-up" section).
 *
 * @param fb Framebuffer to modify, at least HP41_DM41X_FB_SIZE bytes.
 ...
 *                  HP41_DM41X_SEG_DOT/_SEP/_COLON).
 */
static void plot_segment(uint8_t *fb, int cell_x0, int cell_y0, int seg_index)
{
    assert(fb != NULL);
    assert(seg_index >= 0 && seg_index < HP41_DM41X_NUM_SEGMENTS);
    uint16_t off = hp41_dm41x_segment_pixel_offset[seg_index];
    uint16_t cnt = hp41_dm41x_segment_pixel_count[seg_index];
    for (uint16_t k = 0; k < cnt; k++) {
        hp41_dm41x_pixel_t p = hp41_dm41x_segment_pixels[off + k];
 ...
 *
 * @param fb      Framebuffer to modify, at least HP41_DM41X_FB_SIZE bytes.
 * @param ascii   Character code, 0-127 (only entries hp41_dm41x_char_segments
...
static void plot_char(uint8_t *fb, int ascii, int cell_x0, int cell_y0)
{
    ...
    uint16_t segbits = hp41_dm41x_char_segments[ascii];
    for (int b = 0; b < 14; b++)
        if (segbits & (1u << b))
 ...
 * @param row Grid row, 0 to HP41_DM41X_NUM_ROWS-1.
 */
static void plot_formatted_row(uint8_t *fb, const hp41_formatted_number_t *f, int row)
{
    assert(fb != NULL);
    assert(f != NULL);
    assert(row >= 0 && row < HP41_DM41X_NUM_ROWS);
    assert(f->num_fields <= HP41_DM41X_NUM_COLS);

    int row_y0 = HP41_DM41X_GRID_Y0 + row * HP41_DM41X_ROW_PITCH_PX;
    for (int col = 0; col < f->num_fields; col++) {
        int cell_x0 = HP41_DM41X_GRID_X0 + col * HP41_DM41X_COL_PITCH_PX;
        ...
        if (field->decimal_point_after)
            plot_segment(fb, cell_x0, row_y0, HP41_DM41X_SEG_DOT);
        if (field->separator_after)
            plot_segment(fb, cell_x0, row_y0, HP41_DM41X_SEG_SEP);
    }
}
...
static void render_classic_line_view(uint8_t *fb)
{
    ...
    for (int pos = 0; pos < HP41_DM41X_NUM_CLASSIC_CELLS; pos++) {
        /* lcd_*[11] is the leftmost screen position, [0] the rightmost -
         * matches hp41_display_bridge.c's own identical convention. */
        int i = (HP41_DM41X_NUM_CLASSIC_CELLS - 1) - pos;
        int v = (lcd_c[i] << 8) | ((lcd_b[i] & 3) << 4) | lcd_a[i];
        int ascii = hp41_decode_ascii(v) & 0x7f;
        int punct = lcd_b[i] >> 2;
        int cell_x0 = HP41_DM41X_GRID_X0 + pos * HP41_DM41X_COL_PITCH_PX;

        plot_char(fb, ascii, cell_x0, HP41_DM41X_GRID_Y0_CLASSIC);

        switch (punct) {
            case 1: /* period */
                plot_segment(fb, cell_x0, HP41_DM41X_GRID_Y0_CLASSIC, HP41_DM41X_SEG_DOT);
                break;
            case 2: /* colon */
                plot_segment(fb, cell_x0, HP41_DM41X_GRID_Y0_CLASSIC, HP41_DM41X_SEG_COLON);
                break;
            case 3: /* comma */
                plot_segment(fb, cell_x0, HP41_DM41X_GRID_Y0_CLASSIC, HP41_DM41X_SEG_SEP);
                break;
            default:
                break;
        }
     ...
     * silently dropped - see CLAUDE.md's "DM41X display bridge" section. */
    (void)lcd_ann;
}

void hp41_dm41x_display_compute_framebuffer(uint8_t *fb, hp41_dm41x_view_t view)
{
    assert(fb != NULL);
    memset(fb, 0xFF, HP41_DM41X_FB_SIZE);

    switch (view) {
        case HP41_DM41X_VIEW_STACK:
            render_stack_view(fb);
            break;
        case HP41_DM41X_VIEW_CLASSIC_LINE:
            render_classic_line_view(fb);
            break;
        default:
            assert(0 && "unknown hp41_dm41x_view_t");
            break;
    }
}
