...
/* Set by "[DSP]" (Phase 3c of the Magellan/QUAD plan), consumed (and
 * cleared) by hp41_key_bridge_quad_view_toggle_requested() - same
 * one-shot-flag rationale as clear_memory_requested above. */
static bool quad_view_toggle_requested = false;
...
void hp41_key_bridge_reset(void)
{
    ...
    quad_view_toggle_requested = false;
    leet_progress = 0;
...
{
    bool requested = quad_view_toggle_requested;
    quad_view_toggle_requested = false;
    assert(quad_view_toggle_requested == false);
    return requested;
}
    ...
    if (c == ']') {
        if (name_len >= 0) {
            ...
            if (name_buf[0] == '-') {
                ...
                /* Bridge-level command, not a real key - see
                 * hp41_key_bridge_quad_view_toggle_requested()'s header
                 * doc. Same "just set a flag" rationale as CLRMEM above. */
                quad_view_toggle_requested = true;
