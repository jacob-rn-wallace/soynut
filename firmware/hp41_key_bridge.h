...
// Feeds one incoming byte from the USB serial keypress stream into the
// Nut CPU's keybuffer[]. Pure logic (no hardware/USB access) - safe to
// call/test on a host build.
//
...
// Resets the bridge's internal "[NAME]" escape-sequence state to idle.
